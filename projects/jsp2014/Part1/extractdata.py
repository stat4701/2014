...
for db in databases:
    ...
    hands = all_hands.PLAYER_ID.value_counts()
    bb_won = all_hands.groupby('PLAYER_ID').AMT_WON.sum()
    evbb_won = all_hands.groupby('PLAYER_ID').EV_WON.sum()
    players = pd.concat((hands, bb_won, evbb_won), axis=1)
    players = players.join(screennames, how = 'left')

    ## make number of hands played a float
    make_float = lambda x: float(x)
    players[0] = players[0].apply(make_float)
