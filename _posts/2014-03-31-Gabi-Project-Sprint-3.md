...
* I looked at correlations between different variables by looking at heat maps and scatter plots, and I created maps of obesity percentages. 
...
> I plotted a few heat maps using various colors which all showed the highest correlation between obesity and no exercise as well as high blood pressure and diabetes. 
...
> I think the pink map allows you to see that there are actually quite a few states with obesity between 27 % and 31 % since these are colored darker than in the other map where they are light blue and this could possibly be misleading. In contrast, I think the orange map makes it easier to see that most states reported obesity between 23 and 27 percent.
...
> Next, I subsetted risk data to only include states with obesity over 10% and 20% and put it on the map using State data:
I used the Paired palette from colorbewer which shows different colors paired, and I think it does a nice job at showinghow some states relate to others:
...
### Progress I made using ggplot for Maps - County Level:

> Next I wanted to plot obesity at the county level and created a few more maps. To do this, I created a dataframe by merging risk data and county data (obtained through the 'map_data' function mentioned above) by county and state name. When looking at the maps, I was surprised to see so many white areas. For example, in the map where obesity is colored by state in Texas, it looks as though the entire state is between 23-28 percent obese, but when looking at the map where counties are used Texas is very white. Looking back at the original risk dataset, 190 of 254 total records from Texas did not have data (rows values were -1111) which explains that the large number of white areas are due to missing data for those counties.
...
* Figure out how to work with the data in a meaningful way and what conclusions can be drawn from the other datasets.
