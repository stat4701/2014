...
test_that("vmode upgrading works (byte to short)",{
    foo <- .compact(ffdf(foo=ff(c(-125,125))))
    bar <- data.frame(foo=150)
    baz <- ffStack(foo,bar)
    expect_equal(vmode(foo[[1]][1]), "byte")
    expect_equal(vmode(baz[[1]][1]), "short")
})

test_that("dates work across ffdf and data.frame", {
